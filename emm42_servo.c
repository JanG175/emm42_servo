...
static uint8_t crc = CRC_0x6B; // CRC mode
 ...
 */
static uint8_t emm42_uart_calc_CRC(uint8_t* datagram, uint32_t len, uint8_t crc_type)
{
    uint8_t crc = 0x00;

    if (crc_type == CRC_0x6B)
        crc = 0x6B;
 ...
 */
static bool emm42_uart_check_CRC(uint8_t* datagram, uint32_t len, uint8_t crc_type)
{
    uint8_t crc = emm42_uart_calc_CRC(datagram, len, crc_type);
 ...
 * 
 * @param emm42_config struct with EMM42 connection parameters
 * @param datagram pointer to datagram
 * @param len length of datagram
 */
static void emm42_uart_send(emm42_config_t emm42_config, uint8_t* datagram, uint8_t len)
{
    uart_write_bytes(emm42_config.uart, (const uint8_t*)datagram, len);
    ESP_ERROR_CHECK(uart_wait_tx_done(emm42_config.uart, UART_TIMEOUT_MS));
}
 ...
 * @param len length of datagram
 */
static void emm42_uart_recv(emm42_config_t emm42_config, uint8_t* datagram, uint8_t len)
{
    uint32_t buf = 0;
    uint8_t data[len];

    buf = uart_read_bytes(emm42_config.uart, data, len, UART_TIMEOUT_MS);
    uart_flush(emm42_config.uart);
...
void emm42_init(emm42_config_t emm42_config)
{
    ...
    uart_config_t uart_config = {
        .baud_rate = emm42_config.baudrate,
    ...
    ESP_ERROR_CHECK(uart_driver_install(emm42_config.uart, 2048, 2048, 0, NULL, 0));
    ESP_ERROR_CHECK(uart_param_config(emm42_config.uart, &uart_config));
    ESP_ERROR_CHECK(uart_set_pin(emm42_config.uart, emm42_config.tx_pin, emm42_config.rx_pin, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
    ...
    for (uint32_t i = 0; i < timer_N; i++)
    {
        ...
        io_conf.pin_bit_mask = ((1 << emm42_config.step_pin[i]) | (1 << emm42_config.dir_pin[i]) | (1 << emm42_config.en_pin[i]));
        io_conf.pull_down_en = 0;
        io_conf.pull_up_en = 0;
        gpio_config(&io_conf);

        emm42_enable(emm42_config, i, 0);
        emm42_set_dir(emm42_config, i, CW_DIR);
        ...
        cb_arg[i].step_pin = emm42_config.step_pin[i];
        ...
        emm42_enable(emm42_config, i, 1); // enable motor
    }
}
...
void emm42_deinit(emm42_config_t emm42_config)
{
    ...
    ESP_ERROR_CHECK(uart_driver_delete(emm42_config.uart));
}
...
{
    if (enable == 0)
        gpio_set_level(emm42_config.en_pin[motor_num], 0);
    else if (enable == 1)
        gpio_set_level(emm42_config.en_pin[motor_num], 1);
}
 ...
 */
void emm42_set_dir(emm42_config_t emm42_config, uint32_t motor_num, uint32_t dir)
{
    if (dir == CW_DIR)
        gpio_set_level(emm42_config.dir_pin[motor_num], 0);
    else if (dir == CCW_DIR)
        gpio_set_level(emm42_config.dir_pin[motor_num], 1);
}
...
void emm42_start(emm42_config_t emm42_config, uint32_t motor_num, uint32_t start)
{
    if (start == 0)
    {
        ...
        gpio_set_level(emm42_config.step_pin[motor_num], 0);
    }
    else if (start == 1)
    {
...
void emm42_step_move(emm42_config_t emm42_config, int64_t* steps, uint32_t* period_us)
{
    ...
    {
        emm42_start(emm42_config, motor_num, 0);

        // set direction
        if (steps[motor_num] < 0)
        {
            emm42_set_dir(emm42_config, motor_num, 0);
            steps[motor_num] = -steps[motor_num];
        }
        else
            emm42_set_dir(emm42_config, motor_num, 1);

        // set period
        emm42_set_period(motor_num, period_us[motor_num]);
    ...
    for (uint32_t motor_num = 0; motor_num < timer_N; motor_num++)
        emm42_start(emm42_config, motor_num, 1);
 ...
 * 
 * @param emm42_config struct with emm42 configs
...
void emm42_uart_calibrate_encoder(emm42_config_t emm42_config, uint8_t address)
{
    ...
    datagram[1] = 0x06;
    datagram[2] = 0x45;
    datagram[3] = emm42_uart_calc_CRC(datagram, len_w, crc);

    emm42_uart_send(emm42_config, datagram, len_w);
...
float emm42_uart_read_encoder(emm42_config_t emm42_config, uint8_t address)
{
    ...
    datagram[0] = address;
    datagram[1] = ENCODER_READ;
    datagram[2] = emm42_uart_calc_CRC(datagram, len_w, crc);
    ...
    do
    {
        emm42_uart_recv(emm42_config, response, len_r);
        cnt++;
    } while ((response[0] != address || emm42_uart_check_CRC(datagram, len_r, crc) != false) && (cnt < UART_MAX_REPEAT));
...
void emm42_uart_turn(emm42_config_t emm42_config, uint8_t address, int16_t speed, uint8_t accel)
{
    ...
    datagram[0] = address;
    datagram[1] = TURN_FUNC;
    datagram[2] = speedH;
    datagram[3] = speedL;
    datagram[4] = accel;
    datagram[5] = emm42_uart_calc_CRC(datagram, len_w, crc);
...
void emm42_uart_move(emm42_config_t emm42_config, uint8_t address, int16_t speed, uint8_t accel, uint32_t pulse)
{
    ...
    datagram[6] = pulseM;
    datagram[7] = pulseL;
    datagram[8] = emm42_uart_calc_CRC(datagram, len_w, crc);
